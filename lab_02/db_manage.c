    ...
    for (int i = 0; i < n_flats; ++i) {
        if (flats[i].housing_type == 1)
            fprintf(db, "%s,%d,%d,%ld,%d,%d,\n", flats[i].address, flats[i].area, flats[i].n_rooms,
                    flats[i].cost_per_square, flats[i].housing_type, flats[i].housing.decoration);
        else
            fprintf(db, "%s,%d,%d,%ld,%d,%d,%d,%d,\n", flats[i].address, flats[i].area, flats[i].n_rooms,
                    flats[i].cost_per_square, flats[i].housing_type, flats[i].housing.secondary.year,
                    flats[i].housing.secondary.n_prev_owners, flats[i].housing.secondary.animals);
    }
}
...
void print_db_header()
{
    printf("┌──────┬────────────────────────────────────────────────────┬────────┬──────────┬─────────────────┬────────"
           "──────┬────────────┬──────┬─────────┬─────────┐\n"
           "│ Numb │                     Address                        │  Area  │ Rooms n. │ Cost per square │ "
           "Housing type │ Decoration │ Year │ N. owns │ Animals │\n"
           "├──────┼────────────────────────────────────────────────────┼────────┼──────────┼─────────────────┼────────"
           "──────┼────────────┼──────┼─────────┼─────────┤\n");
}
...
{
    printf("│ %4d ", n_record);
    printf("│ %-50s │ %6d │ %8d │ %12ld p. │ %-12s │", flat->address, flat->area, flat->n_rooms, flat->cost_per_square,
           flat->housing_type == 1 ? "Primary" : "Secondary");
    if (flat->housing_type == 1)
        printf(" %-10s │ %6s │ %9s │ %-9s │\n", flat->housing.decoration == 0 ? "No" : "Yes", "—", "—", "—");
    else
        printf(" %-12s │ %4d │ %7d │ %-7s │\n", "—", flat->housing.secondary.year,
               flat->housing.secondary.n_prev_owners, flat->housing.secondary.animals == 0 ? "No" : "Yes");
}
