...
{
    printf("│ %11zu │ %14zu │ %14lf │ %14zu │ %14lf │\n", a2->n_processed, q1_n_requests,
           (double)a1->length_sum / total_work_time, q2_n_requests, (double)a2->length_sum / total_work_time);
}
...
void print_all_info(machine_t *a1, machine_t *a2, double total_work_time)
{
    ...
    printf("│ %-60s %12zu\n", "Number of requests processed by the first machine:", a1->n_processed);
    printf("│ %-60s %12lf\n",
           "Average time for the requests to stay in the first queue:", a1->request_time_sum / a1->n_processed);
    printf("│ %-60s %12lf\n│ \n",
           "Average time for the requests to stay in the second queue:", a2->request_time_sum / a2->n_processed);
}
...
void update_stat(size_t n_requests_q1, size_t n_requests_q2, machine_t *a1, machine_t *a2, double *total_work_time,
                 stat_flag f)
{
    double time_left;
...
void handle_simultaneous(queue_t *q1, queue_t *q2, machine_t *a1, machine_t *a2, double *total_work_time,
                         double possibility)
{
...
void process(queue_t *q1, queue_t *q2, machine_t *a1, machine_t *a2, double possibility)
{
    ...
    while (a2->n_processed < MAX_N_REQUESTS) {
        ...
        else if (q1->Get_n_requests(q1) != 0 && q2->Get_n_requests(q2) != 0 && a1->time_left < EPS &&
                 a2->time_left < EPS) {
            assert(q1->Pop(q1, &a1->r) >= 0);
            a1->request_time_sum += total_work_time - a1->r.entered_time;
            a1->time_left = get_time(a1->time_min, a1->time_max);
         ...
         */
        else if (a1->time_left >= EPS && a2->time_left < EPS && q1->Get_n_requests(q1) != 0 &&
                 q2->Get_n_requests(q2) != 0) {
