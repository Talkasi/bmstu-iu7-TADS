...
void free_all(mbc *mbc1, mbr *mbr2, mbr *mbr_res, mtx *mtx1, mtx *mtx2, mtx *mtx_res);
...
{
    char *menu[] = {
        "0 - Exit", "1 - Input matrixes", "2 - Measure efficiency", "3 - Print matrixes", "4 - Multiply matrixes",
    };
    ...
    while (command != EXIT) {
        ...
        case MUL_DATA: {
            ...
            if (rc == MALLOC_ERR)
                printf("Error while allocationg memory. Matrix is too big.\n");
...
int scan_mtx_from_keyboard(mtx *m, long needed_size)
{
    ...
    if (realloc_mtx(m, n_r, n_c) != 0) {
        printf("Too big mtx. Couldn't allocate memory.\n");
        return MALLOC_ERR;
    }
    mtx_init0(m);
    ...
    for (long i = 0; i < n; ++i) {
        ...
        cc -= 1;
        cr -= 1;
        while (read_int(&m->nums[cr * n_c + cc]) != 0 || m->nums[cr * n_c + cc] == 0) {
...
void print_eff(ull *avg_mtx, ull *avg_mb, size_t *mem_mtx, size_t *mem_mb)
{
    printf("┌──────────────┬───────────────────────────┬─────────────────────────────┬──────────────────┬──────────────"
           "──────┐\n"
           "│  %% of sparse │ Time of mul ord. matrixes │ Time of mul sparse matrixes │    Memory ord.   │   Memory "
           "sparse    │\n"
           "├──────────────┼───────────────────────────┼─────────────────────────────┼──────────────────┼──────────────"
           "──────┤\n");
    for (int i = 0; i < MAX_PERSENT; i += STEP)
        printf("│ %11d%% │ %22lld us │ %24lld us │ %10ld bytes │ %12ld bytes │\n", i, avg_mtx[i], avg_mb[i], mem_mtx[i],
               mem_mb[i]);
