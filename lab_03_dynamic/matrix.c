            ...
            *(res->nums + i * res->n_cols + j) = 0;
            for (long k = 0; k < m1->n_cols; ++k)
                *(res->nums + i * res->n_cols + j) +=
                    *(m1->nums + i * m1->n_cols + k) * *(m2->nums + k * m2->n_cols + j);
        }
...
int mul_mbc_by_mbr(mbc *m1, mbr *m2, mbr *res)
{
    ...
    if (realloc_mbr(res, MIN((m1->non_zero_n * m2->non_zero_n), (m1->n_of_rows * m2->n_of_cols)),
    ...
    while (1) {
        if (m1->f_in_rows[cur_f1] >= 0 && m2->f_in_cols[cur_f2] >= 0) {
            while (are_nums_in_m1(m1, cur_f1, i1) && are_nums_in_m2(m2, cur_f2, i2)) {
